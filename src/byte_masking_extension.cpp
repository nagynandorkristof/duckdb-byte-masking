			...
			if (has_endianness_param) {
				...
				if (endian_format.validity.RowIsValid(endian_idx)) {
					...
					if (endian_str == "big") {
						...
						throw InvalidInputException("Invalid endianness parameter. Must be 'big' or 'little', got: " +
						                            endian_str);
					}
				}
			}
...
static void LoadInternal(ExtensionLoader &loader) {
	...
	auto mask_bytes_function_full =
	    ScalarFunction("mask_bytes", {LogicalType::VARCHAR, LogicalType::BLOB, LogicalType::VARCHAR},
	...
	auto mask_bytes_function_simple =
	    ScalarFunction("mask_bytes", {LogicalType::VARCHAR, LogicalType::BLOB},
	                   LogicalType::MAP(LogicalType::VARCHAR, LogicalType::BIGINT), MaskBytesScalarFun);
